...
function searchAll(keyword)
{
	...
	if (keyword.length>90) keyword=keyword.substring(0,90)+"..."
	...
	document.write("<center><h3>Resultados Econtrados <b>"+(display_start+1)+"-"+(displast)+"</b> de <b>"+sites+"</b> para <b>"+keyword+"</b> Tiempo de búsqueda <b>"+((timeB-timeA)/1000)+"</b> segundos.</h3></center>")
	...
	for (q=display_start; q<q3; q+=1) {
		if (results[q].val>0) {
			...
			if (x1>0 && x2>0) {
				...
				document.write("<table border=0 width=400><tr><td><span class='xresult'>"+records[rc].description+"</span></td></tr></table><span class='xlocation'>"+tmp+"</span><br><br>")
			}
			q2++
   		}
	}
